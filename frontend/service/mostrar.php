                ...
                <div class="row align-items-center">
                    <div class="col-5">
                        ...
                        <div class="col-4">
                          <form method="POST" autocomplete="off" action="" style="margin:auto;max-width:600px">
                            <input type="searchs" class="form-control form-control-line"  placeholder="Buscar servicio ..." name="keyword" value="<?php echo isset($_POST['keyword']) ? $_POST['keyword'] : '' ?>" required=""/>
                    ...
                    <div class="col-7">
                        <div class="text-end upgrade-btn">
                            <a href="../service/nuevo.php" class="btn btn-danger text-white"
...
<?php
        ...
        $query = $connect->prepare("SELECT service.idserv, suscrp.idsus, suscrp.noms, suscrp.descr, suscrp.foto, service.nameserv, service.prec, service.cont, service.state FROM service INNER JOIN suscrp ON service.idsus = suscrp.idsus WHERE `noms` LIKE '%$keyword%' or  `descr` LIKE '%$keyword%' or  `nameserv` LIKE '%$keyword%' or  `prec` LIKE '%$keyword%'  GROUP BY service.idserv");
        $query->execute();
  ...
  <div class="col mb-4">
    <div class="card">
      <h4 class="card-header">
        <?php echo  $row->noms; ?>
            <a href="../service/delete.php?id=<?php echo  $row->idserv; ?>" style="color: #fff;"  type="button" class="btn btn-block btn-danger"><i class="mdi mdi-close-circle"></i></a>
        </h4>
      <div class="card-body">
        <p><?php echo  $row->nameserv; ?></p>
        <ul class="list-unstyled">
          <li class="list-item"><strong>Precio:</strong> <?php echo  $row->prec; ?></li>
         
         
          <li><img width='150' height="150" src="../../backend/img/subidas/<?php echo  $row->foto; ?>"></li>
        </ul>
        <a type="button" class="btn btn-block btn-primary" href="../service/editar.php?id=<?php echo  $row->idserv; ?>">Actualizar</a>
      </div>
    </div>
  </div>
...
<?php
        $query = $connect->prepare("SELECT service.idserv, suscrp.idsus, suscrp.noms, suscrp.descr, suscrp.foto, service.nameserv, service.prec, service.cont, service.state FROM service INNER JOIN suscrp ON service.idsus = suscrp.idsus GROUP BY service.idserv DESC");
        $query->execute();
        while($row = $query->fetch()){
